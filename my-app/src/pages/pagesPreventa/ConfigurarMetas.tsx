...
const API = "http://localhost:5000/api/metas";
...
const FORM_VACIO = {
  preventa: "", meta_unidades: "", meta_dolares: "", mes: "",
...
export default function ConfigurarMetas() {
  ...
  const cargarMetas = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API}/listarmetas`);
  ...
  const guardarMeta = async () => {
    ...
    setGuardando(true);
    try {
      const res = await fetch(`${API}/guardarmetas`, {
        method: "POST",
  ...
  const abrirEditar = (m: Meta) => setEditando({ ...m });

  // ── Confirmar edición ───────────────────────────────
  const confirmarEdicion = async () => {
    if (!editando) return;
    try {
      const res = await fetch(`${API}/editarmeta`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          preventa: editando.codigo_ruta,
          mes: editando.mes,
          anio: editando.anio,
  ...
  const confirmarEliminar = async () => {
    ...
    try {
      const res = await fetch(`${API}/eliminarmeta`, {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          preventa: confirmDel.codigo_ruta,
          mes: confirmDel.mes,
  ...
  const metasFiltradas = metas.filter(
    (m) => m.mes === mesFiltro && m.anio === anioFiltro
  )
    ...
    <div className="min-h-screen bg-[#011f18] text-white">
      ...
      <div className="relative z-10 border-b border-white/5 bg-[#012a20]/60 backdrop-blur-md px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-white leading-tight tracking-tight">Metas de Preventas</h1>
            <p className="text-[11px] text-emerald-400/70 leading-none">Configuración de objetivos por ruta</p>
          </div>
        </div>
        <Link to="/dashboard/preventa"
      ...
      <div className="relative z-10 p-6 grid grid-cols-1 xl:grid-cols-[420px_1fr] gap-6 max-w-[1400px] mx-auto">
        ...
        <div className="bg-[#012a20]/80 border border-white/8 rounded-2xl p-6 shadow-xl backdrop-blur-sm h-fit">
          <div className="flex items-center gap-2 mb-6">
            ...
            <h2 className="text-base font-bold text-white">Nueva Meta</h2>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input
                name="preventa"
                placeholder="Ej: PV1, TELEVENTA 1"
                value={form.preventa}
                onChange={e => setForm({ ...form, preventa: e.target.value.toUpperCase() })}
                className="w-full px-3.5 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white text-sm placeholder-white/20 focus:outline-none focus:border-emerald-500/60 focus:bg-white/8 transition-all"
              />
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <select name="mes" value={form.mes} onChange={e => setForm({ ...form, mes: e.target.value })}
                  className="w-full px-3.5 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white text-sm focus:outline-none focus:border-emerald-500/60 transition-all">
              ...
              <div>
                ...
                <select name="anio" value={form.anio} onChange={e => setForm({ ...form, anio: e.target.value })}
                  className="w-full px-3.5 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white text-sm focus:outline-none focus:border-emerald-500/60 transition-all">
                  {años.map(y => <option key={y} value={y} className="bg-[#012a20]">{y}</option>)}
                </select>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input type="number" name="meta_unidades" placeholder="0"
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="number" name="meta_dolares" placeholder="0.00"
            ...
            <button onClick={guardarMeta} disabled={guardando}
              className="w-full mt-2 flex items-center justify-center gap-2.5 py-3 rounded-xl font-bold text-sm text-black bg-gradient-to-r from-emerald-400 to-teal-500 hover:from-emerald-300 hover:to-teal-400 disabled:opacity-50 transition-all shadow-[0_4px_20px_rgba(13,180,139,0.35)] hover:shadow-[0_4px_28px_rgba(13,180,139,0.5)]">
              {guardando
                ? <span className="w-4 h-4 border-2 border-black/30 border-t-black rounded-full animate-spin" />
                : <BsPlus className="text-lg" />}
              {guardando ? "Guardando…" : "Guardar Meta Preventa"}
            </button>
          ...
          <div className="px-6 py-4 border-b border-white/8 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <select
                ...
                className="bg-[#021f18] border border-white/10 text-white text-xs rounded-lg px-2 py-1"
              >
                {MESES.map((mes, i) => (
          ...
          <div className="overflow-x-auto">
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 text-white/20">
                ...
                <p className="text-sm">No hay metas registradas</p>
              </div>
            ) : (
                      ...
                      <td className="px-5 py-3.5">
                        <div className="flex items-center justify-center gap-2">
                          ...
                          <button onClick={() => abrirEditar(m)} title="Editar"
                            className="w-8 h-8 rounded-lg bg-blue-500/10 hover:bg-blue-500/25 border border-blue-500/20 hover:border-blue-400/40 text-blue-400 hover:text-blue-300 transition-all flex items-center justify-center">
                            <BsPencilSquare className="text-xs" />
                          </button>
                          {/* Eliminar */}
                          <button onClick={() => setConfirmDel(m)} title="Eliminar"
                            className="w-8 h-8 rounded-lg bg-red-500/10 hover:bg-red-500/25 border border-red-500/20 hover:border-red-400/40 text-red-400 hover:text-red-300 transition-all flex items-center justify-center">
      ...
      {editando && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-md bg-[#012a20] border border-white/10 rounded-2xl shadow-2xl p-6 animate-[fadeSlideUp_0.2s_ease]">
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => setEditando(null)}
                className="w-7 h-7 rounded-lg bg-white/5 hover:bg-white/10 text-white/40 hover:text-white transition flex items-center justify-center text-sm">
                ✕
              </button>
            </div>

            {/* Info de identificación (solo lectura) */}
            ...
            <div className="flex gap-3 mt-6">
              <button onClick={() => setEditando(null)}
                className="flex-1 py-2.5 rounded-xl border border-white/10 text-white/50 hover:text-white hover:border-white/20 text-sm font-semibold transition-all">
                Cancelar
              </button>
              <button onClick={confirmarEdicion}
                className="flex-1 py-2.5 rounded-xl bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-400 hover:to-blue-500 text-white text-sm font-bold transition-all shadow-[0_4px_16px_rgba(59,130,246,0.35)] flex items-center justify-center gap-2">
          ...
          <div className="w-full max-w-sm bg-[#012a20] border border-red-500/20 rounded-2xl shadow-2xl p-6">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={confirmarEliminar}
                className="flex-1 py-2.5 rounded-xl bg-gradient-to-r from-red-500 to-red-600 hover:from-red-400 hover:to-red-500 text-white text-sm font-bold transition-all shadow-[0_4px_16px_rgba(239,68,68,0.3)] flex items-center justify-center gap-2">
