...
export default function DetalleClientesCanalDescartablePage() {
  ...
  const exportar = () => {
    const datos = filtrados.map((c, i) => ({
      ...
      "Variación (%)": c.variacion_porc !== null ? `${c.variacion_porc}%` : "—",
      "Última Compra": c.ultima_compra || "—",
      "Con consumo": c.consumo_actual > 0 ? "Sí" : "No",
    }));
            ...
            <div className="md:hidden divide-y divide-[#046C5E]/20">
              {paginados.length === 0
                ...
                : paginados.map((c, idx) => {
                    ...
                    return (
                      <div key={`${c.customer_code}-${c.customer_address_code ?? idx}`}
                        ...
                        <div className="flex items-start justify-between mb-3">
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[11px] text-gray-400 font-mono mt-0.5">{c.customer_code}</p>
                          </div>
                        ...
                        <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-xs mb-2">
                          ...
                          <div>
                            ...
                            <p className={`font-bold ${c.variacion_abs >= 0 ? "text-green-400" : "text-red-400"}`}>
                              {c.variacion_abs >= 0 ? "+" : ""}${fmt(Math.abs(c.variacion_abs))}
                            </p>
                          </div>
                          <div>
                            <p className="text-[10px] text-gray-500 uppercase tracking-wide">Unidades</p>
                            <p className="text-blue-300 font-semibold">{c.unidades_actual}</p>
                          </div>
                          <div>
                ...
                <thead>
                  <tr className="bg-[#014434] text-[10px] uppercase text-green-300 select-none">
                    ...
                    {([
                      ...
                      ["Mes Anterior",    "consumo_anterior"],
                      ["VS Mes Ant",      "variacion_abs"],
                      ["% Variación",     "variacion_porc"],
                      ["Últ. Compra",     "ultima_compra"],
                      ["Estado",          "_estado"],
                    ] as [string, string][]).map(([label, key]) => (
                      <th key={key} onClick={() => key !== "_estado" && requestSort(key)}
                        className={`px-3 py-3 text-left transition-colors whitespace-nowrap ${key !== "_estado" ? "cursor-pointer hover:text-white" : ""}`}>
                        {label}<span className="ml-1 text-[#046C5E]">{key !== "_estado" ? sa(key) : ""}</span>
                      </th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {paginados.length === 0
                    ...
                    : paginados.map((c, idx) => {
                        const sinConsumo = c.consumo_actual === 0;
                        return (
                          <tr key={`${c.customer_code}-${c.customer_address_code ?? idx}`}
                            ...
                            <td className="px-3 py-2 font-mono text-xs text-gray-300">{c.customer_code}</td>
                            ...
                            <td className="px-3 py-2 text-gray-300 text-xs whitespace-nowrap">{c.telefono || "—"}</td>
                            <td className="px-3 py-2 text-right text-blue-300 font-semibold">{c.unidades_actual.toLocaleString("es-EC")}</td>
                            ...
                            <td className={`px-3 py-2 text-right text-xs ${c.variacion_abs >= 0 ? "text-green-400" : "text-red-400"}`}>
                              {c.variacion_porc !== null ? `${c.variacion_porc >= 0 ? "+" : ""}${c.variacion_porc.toFixed(1)}%` : "—"}
                            </td>
