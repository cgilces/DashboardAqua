...
export default function DetalleClientesEmpresasPage() {
  ...
  const [productos,  setProductos]  = useState<{ producto: string; unidades: number; monto: number }[]>([]);
  ...
  useEffect(() => {
    setCargando(true);
    fetch(`${API_BASE_URL}/api/botellones/clientes-empresas?anio=${anio}&mes=${mes}`)
      .then(r => r.json())
      .then(data => {
  ...
  const exportar = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Empresas Botellón");
    XLSX.writeFile(wb, `clientes_empresas_botellon_${anio}_${mes}.xlsx`);
  };
      ...
      <div className="main-content min-h-screen text-white px-4 md:px-8 py-4 md:py-6">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-6 border-b border-[#046C5E]/50 pb-4">
          <div>
            ...
            <p className="text-xs text-gray-400">{MESES[Number(mes)]} {anio} · Consolidado MobilVendor + Odoo</p>
          </div>
                ...
                <tbody>
                  ...
                  <tr className="bg-[#014434] font-bold border-t border-[#046C5E]/30">
                    ...
                    <td className="px-4 py-3 text-right text-green-400">
                      {productos.reduce((a, p) => a + p.unidades, 0).toLocaleString("es-EC")}
                    </td>
                    <td className="px-4 py-3 text-right text-blue-400">
                      ${fmt(productos.reduce((a, p) => a + p.monto, 0))}
                    </td>
                  </tr>
                </tbody>
              </table>
                      ...
                      <div key={c.codigo_cliente}
                        ...
                        <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-xs mb-2">
                          ...
                          <div>
                            ...
                            <p className="text-gray-300">{c.telefono || "—"}</p>
                          </div>
                        </div>
                    ...
                    : paginados.map((c, idx) => {
                        ...
                        return (
                          <tr key={c.codigo_cliente}
                            ...
                            <td className="px-3 py-2 text-gray-400 text-xs">{c.latitud || "—"}</td>
