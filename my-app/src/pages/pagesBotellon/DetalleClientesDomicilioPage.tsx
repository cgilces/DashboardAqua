...
export default function DetalleClientesDomicilioPage() {
  ...
  const [clientes,   setClientes]   = useState<ClienteDomicilio[]>([]);
  ...
  const requestSort = (key: string) => {
    ...
    setClientes(prev => [...prev].sort((a, b) => {
      const av: any = key === "vsAnt" ? (Number((a as any).consumo_actual) - Number((a as any).consumo_anterior)) : (a as any)[key];
      const bv: any = key === "vsAnt" ? (Number((b as any).consumo_actual) - Number((b as any).consumo_anterior)) : (b as any)[key];
  ...
  useEffect(() => {
    setCargando(true);
    fetch(`${API_BASE_URL}/api/botellones/clientes-domicilio?anio=${anio}&mes=${mes}&tipoProducto=${tipoProducto}`)
      .then(r => r.json())
      .then(data => {
  ...
  const exportar = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Domicilio Botellón");
    XLSX.writeFile(wb, `clientes_domicilio_botellon_${anio}_${mes}.xlsx`);
  };
      ...
      <div className="main-content min-h-screen text-white px-4 md:px-8 py-4 md:py-6">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-6 border-b border-[#046C5E]/50 pb-4">
          <div>
            ...
            <h1 className="text-xl md:text-2xl font-bold tracking-tight">Clientes Domicilio — Botellón</h1>
                        ...
                        <div key={c.codigo_cliente}
                          ...
                          <div className="p-4">
                            ...
                            <div className="grid grid-cols-2 gap-2 mb-3">
                              ...
                              <div className="text-center">
                                <p className="text-[9px] text-white/40 uppercase">Cant. Actual</p>
                                <p className="text-sm font-bold text-green-400">{c.cantidad_actual}</p>
                              </div>
                              <div className="text-center">
                                <p className="text-[9px] text-white/40 uppercase">Tipo Negocio</p>
                                <p className="text-xs text-white/60 truncate">{c.tipo_negocio || "—"}</p>
                              </div>
                            </div>
                            <div className="flex items-center justify-between pt-2 border-t border-[#046C5E]/20 text-[10px] text-white/40">
                              <span className="truncate flex-1">{c.direccion_entrega || c.telefono || "—"}</span>
                    ...
                    : paginados.map((c, idx) => {
                        ...
                        return (
                          <tr key={c.codigo_cliente}
                            ...
                            <td className="px-3 py-2 text-white/50 text-xs whitespace-nowrap">{c.telefono || "—"}</td>
