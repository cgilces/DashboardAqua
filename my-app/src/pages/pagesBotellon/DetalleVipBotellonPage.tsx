...
export default function DetalleVipBotellonPage() {
  ...
  useEffect(() => {
    setCargando(true);
    fetch(`${API_BASE_URL}/api/botellones/clientes-vip?anio=${anio}&mes=${mes}`)
      .then(r => r.json())
      .then(data => {
  ...
  const exportar = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "VIP Botellón");
    XLSX.writeFile(wb, `clientes_vip_botellon_${anio}_${mes}.xlsx`);
  };
