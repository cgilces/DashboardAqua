...
interface Props {
  titulo: string;
  endpoint: string;  // ej: "clientes-quito"
  excelSheet: string;
  excelFile: string;
}
...
export default function DetalleClientesCanalBotellonPage({ titulo, endpoint, excelSheet, excelFile }: Props) {
  ...
  const [clientes,   setClientes]   = useState<ClienteCanal[]>([]);
  ...
  useEffect(() => {
    setCargando(true);
    fetch(`${API_BASE_URL}/api/botellones/${endpoint}?anio=${anio}&mes=${mes}&tipoProducto=${tipoProducto}`)
      .then(r => r.json())
      .then(data => {
  ...
  }, [anio, mes, endpoint, tipoProducto]);
  ...
  const exportar = () => {
    ...
    XLSX.writeFile(wb, `${excelFile}_${anio}_${mes}.xlsx`);
  };
