...
export default function EmpresasClientesPorTipoPage() {
  ...
  useEffect(() => {
    setCargando(true);
    fetch(`${API_BASE_URL}/api/botellones/empresas-clientes-tipo?tipo=${encodeURIComponent(tipoDecoded)}&anio=${anio}&mes=${mes}&tipoProducto=${tipoProducto}`)
      .then(r => r.json())
  ...
  const exportar = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Empresas Clientes");
    XLSX.writeFile(wb, `empresas_${tipoDecoded}_${anio}_${mes}.xlsx`);
  };
      ...
      <div className="main-content min-h-screen text-white px-4 md:px-8 py-4 md:py-6">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-6 border-b border-[#046C5E]/50 pb-4">
          <div>
            ...
            <p className="text-xs text-gray-400">Empresas Botellón · {MESES[Number(mes)]} {anio}</p>
          </div>
