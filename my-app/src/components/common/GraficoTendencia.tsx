...
// Gráfico de tendencia 6 meses — reutilizable en todos los dashboards
...
const TooltipTendencia = ({ active, payload, label }: any) => {
  ...
  return (
    <div className="bg-[#010f0c] border border-[#046C5E]/70 rounded-xl px-4 py-3 text-xs text-white shadow-2xl min-w-[170px]">
...
const GraficoTendencia: React.FC<Props> = ({ datos, subtitulo }) => {
  ...
  const maxRef = Math.max(...datos.map(d => Math.max(d.dolares, d.proyeccion ?? d.dolares)));
  const maxUni = Math.max(...datos.map(d => d.unidades));

  // Punto actual para mostrar KPI rápido en header
  const puntoActual = datos.find(d => d.mes === mesActual && d.anio === anioActual);
    ...
    <div className="bg-gradient-to-br from-[#011f1a] to-[#012E24] border border-[#046C5E]/30 rounded-2xl p-5 mb-6 shadow-xl overflow-hidden">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 mb-4">
        <div>
          ...
          <p className="text-[11px] text-gray-500 mt-0.5">{subtitulo ?? "Últimos 6 meses"}</p>
        </div>
      ...
      {/* Strip mensual */}
      <div
        className="grid mt-1 border-t border-[#046C5E]/20 pt-3 gap-1"
        style={{ gridTemplateColumns: `repeat(${datos.length}, 1fr)` }}
      >
        {datos.map((d) => {
