// components/ComponentPlus/TablaPlusOdoo.tsx
...
export default function TablaPlusOdoo({ anio, mes, onTotalesLoaded, onLoadingChange }: Props) {
  const { user } = useAuth();
  const isAdmin = (user?.role ?? "").toUpperCase() === "ADMIN";
  const navigate = useNavigate();

  const [datos, setDatos] = useState<DatosPlusOdoo | null>(null);
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/api/odoo/plus?anio=${anio}&mes=${mes}`)
      .then(res => {
        if (!res.ok) throw new Error("Error Plus");
        return res.json();
      })
      .then((data: DatosPlusOdoo) => {
        setDatos(data);
        ...
        if (onTotalesLoaded) {
          onTotalesLoaded({
            ...
            proyeccionUnidades: data.totales.proyeccion_unidades,
          });
        }
      })
  ...
  const exportarExcel = () => {
    ...
    try {
      ...
      XLSX.utils.sheet_add_aoa(ws, [[`PLUS — VENTAS EMPRESA - ${mes}/${anio}`], []], { origin: "A1" });
      XLSX.utils.sheet_add_json(ws, [row], { origin: "A3" });

      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, ws, "Plus");
      XLSX.writeFile(wb, `plus_odoo_${mes}_${anio}.xlsx`, { compression: true });
        ...
        <table className="min-w-full text-sm">
          ...
          <tbody>
            <tr
              ...
              onClick={() => navigate(`/plus-odoo/clientes/${anio}/${mes}`)}
            >
